...
{% hint style="warning" %}
При использовании **onsite tracking,** важным отличием является передача дерева категорий товаров в виде строки , где в качестве разделителя между категориями используется косая черта \(слеш\), например **`"Clothes/Women/Skirts"`**
{% endhint %}
