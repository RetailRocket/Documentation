...
Товарная база содержит информацию о предложениях интернет-магазина и их описание. Описание предложений используется для ускорения расчета и вывода актуальной информации в рекомендациях.

#### Способы передачи товарной базы

* **YML-файл** - XML-файл в формате YML;
* **Onsite tracking** - Передача товарной базы через product API;

{% hint style="danger" %}
При использовании **onsite tracking,** важным отличием является передача [дерева категорий товаров](../#derevo-tovarnykh-kategorii) в виде строки, где в качестве разделителя между категориями используется косая черта \(слеш\), например **`"Товары для дома/Кухня/Вилки"`**
{% endhint %}
