...
{% hint style="warning" %}
В зависимости от того как выполнена интеграция перадачи товарной базы, необходимо вызывать один из двух вариантов: `categoryView` или `categoryViewByCategoryPath.`

`categoryView` -- товарная база передается спомощью YML-файла;
...
В YML файле доступно дерево категорий с целочисленными идентификаторами категории, поэтому и товары имеют целочисленные идентификаторы категоий что позволяет использовать такие идентификатор в трекинге пользовательского поведения.
...
При использование product API категория передается как `categoryPath` также она должна быть передана и при просмотре страницы категории.
